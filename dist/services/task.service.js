...
// Create New Task
// ==================================
export const createNewTask = async (userId, input) => {
    const { title, description, isRequired, urgency, earlyReminderMinutes, intervalValue, intervalUnit, repeatCount, startDate, } = input;
    ...
    return await prisma.$transaction(async (tx) => {
        ...
        for (let i = 0; i < repeatCount; i++) {
            ...
            if (earlyReminderMinutes) {
                reminderScheduledAt = new Date(scheduledAt.getTime() - earlyReminderMinutes * 60 * 1000);
            }
...
// ==================================
export const updateOccurrenceStatus = async (userId, occurrenceId, input) => {
    const { status } = input;
    return await prisma.$transaction(async (tx) => {
        ...
        const dateStr = occurrence.scheduledAt.toISOString().split("T")[0];
        ...
        const completedRequired = dayOccurrences.filter((o) => o.isRequired && o.status === "COMPLETED").length;
        const totalOptional = dayOccurrences.filter((o) => !o.isRequired).length;
        const completedOptional = dayOccurrences.filter((o) => !o.isRequired && o.status === "COMPLETED").length;
        let dailyStatus = "NO_ACTIVITY";
        if (totalRequired > 0) {
            dailyStatus =
