...
export const processMissedOccurrences = async () => {
    ...
    try {
        ...
        const affectedUserDates = new Set();
        ...
        for (const entry of affectedUserDates) {
            ...
            const activityStatus = totalRequired > 0 && completedRequired === totalRequired
                ? "SUCCESSFUL"
                : "MISSED";
...
export const initTimezoneCronJobs = () => {
    ...
    cron.schedule("0 * * * *", async () => {
        ...
        for (const user of users) {
            ...
            if (localHour === 0) {
                await processUserLocalMidnight(user.id, userTimezone, user.currentStreak, user.longestStreak);
            }
        }
    });
};
...
// ===============================
const processUserLocalMidnight = async (userId, userTimezone, currentStreak, longestStreak) => {
    ...
    const completedRequired = occurrences.filter((o) => o.isRequired && o.status === OccurrenceStatus.COMPLETED).length;
    const totalOptional = occurrences.filter((o) => !o.isRequired).length;
    const completedOptional = occurrences.filter((o) => !o.isRequired && o.status === OccurrenceStatus.COMPLETED).length;
